...
function ClientsAll() {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      // Fetch all clients (both archived and active) - don't pass archived parameter
      const response = await fetch(`${API_URL}/clients/?user_id=${userProfile._id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleArchiveToggle = async (clientId, currentArchivedStatus) => {
    try {
      const token = await getAccessTokenSilently({
        authorizationParams: {
          audience: "https://personalcfo.com"
      ...
      if (response.ok) {
        await fetchClients();
        alert(`Client ${!currentArchivedStatus ? 'archived' : 'unarchived'} successfully!`);
      } else {
    ...
    <div className="clients-container">
      <div className="clients-header">
        <div className="header-content">
          <h1>All Clients</h1>
          <p className="subtitle">View and manage all your clients</p>
        </div>
          ...
          {clients.map((client) => (
            <div key={client._id || client.id} className="client-card">
              ...
              <div className="client-actions">
                <button
                  className={`client-action-btn ${client.archived ? 'unarchive' : 'archive'}`}
                  onClick={() => handleArchiveToggle(client._id || client.id, client.archived || false)}
                >
